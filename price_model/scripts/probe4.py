import time, numpy as np, pandas as pd, os, threading, resource
...
for cfg in [dict(interactions=0,outer_bags=2),dict(interactions=0,outer_bags=6),dict(interactions=6,outer_bags=6)]:
    t=time.time()
    m=ExplainableBoostingRegressor(random_state=42, n_jobs=1, **cfg)
    m.fit(X,y)
    rss=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024
    print(f"OK {cfg} {time.time()-t:.1f}s maxRSS={rss:.0f}MB", flush=True)
