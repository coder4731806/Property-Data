...
listings data and the price prediction models built on top of it. This is only a subset of data (~30k) from period 23 Jun 2025 to 23 Jun 2026. All prices are AUD.

Visit Alethia.au to get price prediction trained on ~660k data points 
...
Four machine-learning models trained on the listing data above (Linear, LightGBM, KNN, EBM).
Each takes the same inputs a seller fills in when posting a listing — suburb, postcode,
property type, bedrooms, bathrooms, car spaces, and optional land and building size — and
returns an expected price, a 90% price band, and a confidence score (0–100).

`price_model/models_full/` has the trained models (`.joblib`, loadable with `joblib.load`
using the scripts in `price_model/scripts/`). `price_model/cf_artifacts/` has the same models
exported to plain JSON plus a zero-dependency JS evaluator, for serving without Python.

---

## Running the price models

### Quickstart (no setup, pure JS)

```bash
cd price_model/cf_artifacts
node -e "
import('./evaluator.mjs').then(({ estimate }) => {
  const pipeline = JSON.parse(require('fs').readFileSync('pipeline.json'));
  const linear = JSON.parse(require('fs').readFileSync('linear.json'));
  const ebm = JSON.parse(require('fs').readFileSync('ebm.json'));
...
`test_parity.mjs` in the same folder runs a full fixture and confirms the JS output matches
the Python-trained models to <0.01%.

### Python setup (to retrain or use the `.joblib` files directly)
